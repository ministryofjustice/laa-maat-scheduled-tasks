...
class AppealDataServiceTest {
    ...
    void givenUnprocessRecordSheetsThatAreSuccessfullyRetrieved_whenPopulateAndProcessAppealDataIsInvoked_thenDataInToMaatIsPopulated() {
        ...
        when(getRecordSheetsResponse.getRetrievedRecordSheets()).thenReturn(
                List.of(xhibitRecordSheet1, xhibitRecordSheet2));
        ...
        when(appealDataRepository.findAll()).thenReturn(List.of(
                XhibitAppealDataEntity.builder().id(1).filename(xhibitRecordSheet1.getFilename())
                        .data(xhibitRecordSheet1.getData()).build(),
                XhibitAppealDataEntity.builder().id(2).filename(xhibitRecordSheet2.getFilename())
                        .data(xhibitRecordSheet2.getData()).build()
        ));
        when(storedProcedureService.callStoredProcedure(any(), anyList())).thenReturn(
                new StoredProcedureResponse(List.of()));

        appealDataService.populateAndProcessAppealDataInToMaat();

        verify(appealDataRepository, times(1)).saveAll(any());
        verify(xhibitDataService, times(1)).getAllRecordSheets(RecordSheetType.APPEAL);
        verify(storedProcedureService, times(1)).callStoredProcedure(
                StoredProcedure.APPEAL_DATA_TO_MAAT_PROCEDURE, procedureParameters1);
        verify(storedProcedureService, times(1)).callStoredProcedure(
                StoredProcedure.APPEAL_DATA_TO_MAAT_PROCEDURE, procedureParameters2);
        verify(xhibitDataService, times(1)).markRecordSheetsAsProcessed(
                List.of(xhibitRecordSheet1.getFilename(), xhibitRecordSheet2.getFilename()),
    ...
    void givenStoredProcedureFailures_whenHandlingExceptions_FailedRecordsAreMarked() {
        List<StoredProcedureParameter<?>> procedureParameters1 = new ArrayList<>(
                TrialDataService.OUTPUT_PARAMETERS);
        procedureParameters1.add(StoredProcedureParameter.inputParameter("id", 1));
        List<StoredProcedureParameter<?>> procedureParameters2 = new ArrayList<>(
                TrialDataService.OUTPUT_PARAMETERS);
        procedureParameters2.add(StoredProcedureParameter.inputParameter("id", 2));
        ...
        when(storedProcedureService.callStoredProcedure(
                StoredProcedure.APPEAL_DATA_TO_MAAT_PROCEDURE, procedureParameters1)).thenReturn(
                new StoredProcedureResponse(List.of()));
        doThrow(StoredProcedureException.class).when(storedProcedureService)
                .callStoredProcedure(StoredProcedure.APPEAL_DATA_TO_MAAT_PROCEDURE,
        ...
        verify(storedProcedureService, times(1)).callStoredProcedure(
                StoredProcedure.APPEAL_DATA_TO_MAAT_PROCEDURE, procedureParameters2);
        verify(xhibitDataService, times(1)).markRecordSheetsAsProcessed(
                List.of(xhibitRecordSheet1.getFilename()), RecordSheetType.APPEAL);
        verify(xhibitDataService, times(1)).markRecordSheetsAsErrored(
                List.of(xhibitRecordSheet2.getFilename()), RecordSheetType.APPEAL);
    }

    @Test
    void givenStoredProcedureResponse_whenContainsErrorCodeOutput_thenMarkFailed() {
        List<StoredProcedureParameter<?>> procedureParameters = new ArrayList<>(
                AppealDataService.OUTPUT_PARAMETERS);
        ...
        when(storedProcedureService.callStoredProcedure(
                StoredProcedure.APPEAL_DATA_TO_MAAT_PROCEDURE, procedureParameters)).thenReturn(
                errorResponse);
