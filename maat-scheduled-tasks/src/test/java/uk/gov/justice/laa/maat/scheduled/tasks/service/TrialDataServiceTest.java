...
class TrialDataServiceTest {
    ...
    void givenNoUnprocessedRecordSheets_whenPopulateAndProcessTrialDataIsInvoked_thenNoDataInToMaatIsPopulated() {
        when(xhibitDataService.getAllRecordSheets(RecordSheetType.TRIAL)).thenReturn(
                getRecordSheetsResponse);

        trialDataService.populateAndProcessTrialDataInToMaat();

        verify(trialDataRepository, never()).saveAllAndFlush(any());
    ...
    void givenUnprocessRecordSheetsThatAreSuccessfullyRetrieved_whenPopulateAndProcessTrialDataIsInvoked_thenDataInToMaatIsPopulated() {
        ...
        when(trialDataRepository.findAll()).thenReturn(List.of(
                XhibitTrialDataEntity.builder().id(1).filename(xhibitRecordSheet1.getFilename())
                        .data(xhibitRecordSheet1.getData()).build(),
                XhibitTrialDataEntity.builder().id(2).filename(xhibitRecordSheet2.getFilename())
        ...
        verify(trialDataRepository, times(1)).saveAllAndFlush(any());
        verify(xhibitDataService, times(1)).getAllRecordSheets(RecordSheetType.TRIAL);
        verify(storedProcedureService, times(1)).callStoredProcedure(StoredProcedure.TRIAL_DATA_TO_MAAT_PROCEDURE,
    ...
    void givenUnprocessedRecordSheetsThatCannotBeRetrieved_whenPopulateAndProcessTrialDataIsInvoked_thenNoDataInToMaatIsPopulated() {
        ...
        verify(trialDataRepository, times(1)).saveAllAndFlush(Collections.emptyList());
        verify(xhibitDataService, times(1)).getAllRecordSheets(RecordSheetType.TRIAL);
        verify(xhibitDataService, never()).markRecordSheetsAsProcessed(any(), any());
    ...
    void givenStoredProcedureFailures_whenHandlingExceptions_FailedRecordsAreMarked() {
        ...
        when(storedProcedureService.callStoredProcedure(StoredProcedure.TRIAL_DATA_TO_MAAT_PROCEDURE,
                procedureParameters1)).thenReturn(new StoredProcedureResponse(List.of()));
        doThrow(StoredProcedureException.class).when(storedProcedureService)
                .callStoredProcedure(StoredProcedure.TRIAL_DATA_TO_MAAT_PROCEDURE, procedureParameters2);
        ...
        verify(storedProcedureService, times(1)).callStoredProcedure(StoredProcedure.TRIAL_DATA_TO_MAAT_PROCEDURE,
                procedureParameters2);
        verify(xhibitDataService, times(1)).markRecordSheetsAsProcessed(
                List.of(xhibitRecordSheet1.getFilename()), RecordSheetType.TRIAL);
        verify(xhibitDataService, times(1)).markRecordSheetsAsErrored(
                List.of(xhibitRecordSheet2.getFilename()), RecordSheetType.TRIAL);
    }
    ...
    void givenStoredProcedureResponse_whenContainsErrorCodeOutput_thenMarkFailed() {
        ...
        when(storedProcedureService.callStoredProcedure(StoredProcedure.TRIAL_DATA_TO_MAAT_PROCEDURE,
                procedureParameters)).thenReturn(errorResponse);
